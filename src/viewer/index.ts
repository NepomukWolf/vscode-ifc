...
class ViewerController implements vscode.CodeLensProvider {
  ...
  private async render(uri: vscode.Uri, id: number): Promise<void> {
    ...
    const message = await vscode.window.withProgress(
      ...
      async (): Promise<LoadMessage> => {
        ...
        this.lastPickMode = selection.pickMode;
  ...
  private async reveal(target: PickTarget): Promise<void> {
    ...
    try {
      ...
      const sourceId = resolvePickSourceId(index, target, this.lastPickMode);
  ...
  private async focus(target: PickTarget): Promise<void> {
    ...
    try {
      const sourceId = resolveFocusSourceId(target, this.lastPickMode);
